...
void Right_PWM_Init(uint16_t period, uint16_t high){
...
void Right_PWM_Duty(uint16_t high){
  TIMER0_TAMATCHR_R = TIMER0_TAILR_R-high; // duty cycle = high/period
}

void Left_PWM_Init(uint16_t period, uint16_t high){
	volatile unsigned long delay;
  ...
  TIMER1_CTL_R &= ~TIMER_CTL_TBEN; // disable timer1 during setup
  TIMER1_CFG_R = TIMER_CFG_16_BIT; // configure for 16-bit timer mode
                                   // configure for alternate (PWM) mode
  TIMER1_TBMR_R = (TIMER_TAMR_TAAMS|TIMER_TBMR_TBMR_PERIOD);
  TIMER1_TBILR_R = period-1;       // timer start value
  TIMER1_TBMATCHR_R = period-high-1; // duty cycle = high/period
  TIMER1_CTL_R |= TIMER_CTL_TBEN;  // enable timer1 16-b, PWM
}

void Left_PWM_Duty(uint16_t high){
  TIMER1_TBMATCHR_R = TIMER1_TBILR_R-high; // duty cycle = high/period
}
