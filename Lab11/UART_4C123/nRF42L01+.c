...
void SSI_Init(void){
	...
	//Enable CSN and CE
	GPIO_PORTF_DIR_R |= 0x08; //Make PF3 out (CSN)
	GPIO_PORTF_DIR_R &= ~0x10;	//Make PF4 in (IRQ)
	...
	GPIO_PORTF_DEN_R |= 0x07; //Enable digital I/O on PF0,1,2 (MISO, MOSI, SCK)
	...
	//SSI1_CC_R = (SSI1_CC_R&~SSI_CC_CS_M)+SSI_CC_CS_M;
	SSI1_CC_R = (SSI1_CC_R&~SSI_CC_CS_M)+SSI_CC_CS_PIOSC; //Configure for system clock
	SSI1_CPSR_R = (SSI1_CPSR_R&~SSI_CPSR_CPSDVSR_M)+64; //Configure for 250 KHz
...
//	SSI0_CPSR_R = (SSI0_CPSR_R&~SSI_CPSR_CPSDVSR_M)+10; // must be even number
	...
	SSI1_CR0_R &= ~(0x0000FF00 |	//SCR = 0 (250 kbps data rate)
									SSI_CR0_SPH |	//Phase = 0
...
void send_data(uint8_t data){
	...
	SSI1_DR_R = 0x82; //Write to PWR_UP
	while((SSI1_SR_R&SSI_SR_BSY)==SSI_SR_BSY){};	//Wait until SSI1 TX not busy
	SSI1_DR_R = 0x1A; //Write to PRIM_RX
	while((SSI1_SR_R&SSI_SR_BSY)==SSI_SR_BSY){};	//Wait until SSI1 TX not busy
	SSI1_DR_R = data;
